...
with st.sidebar:
    selected = option_menu("Utilities",
                           ["Stocks Performance Comparison", "Real-Time Stock Price", "Stock Prediction", 'About'])
        ...
        if (a == ""):  # if user doesn't select any company
            ...
            def plot_raw_data():  # function for plotting raw data
                ...
                fig.layout.update(  # update layout
                    title_text='Line Chart of {}'.format(a),
                    xaxis_rangeslider_visible=True)  # title, x-axis: rangeslider
    ...
    if (a == ""):  # if user doesn't select any company
        ...
        def plot_raw_data():  # function for plotting raw data
            ...
            fig.add_trace(go.Scatter(  # add scatter plot
                x=data['Date'], y=data['Close'], name="stock_close"))  # x-axis: date, y-axis: close
            fig.layout.update(  # update layout
                title_text='Time Series Data of {}'.format(a),
                xaxis_rangeslider_visible=True)  # title, x-axis: rangeslider
            st.plotly_chart(fig)  # display plotly chart
...
# Stock Price Prediction Section Ends Here
    ...
    st.markdown(
        '<p class="big-font">StockStream is a web application that allows users to visualize Stock Performance Comparison, Real-Time Stock Prices and Stock Price Prediction. This application is developed using Streamlit. Streamlit is an open source app framework in Python language. It helps users to create web apps for Data Science and Machine Learning in a short time. This Project is developed by Vishal Malik. You can find more about the developer on their GitHub Profile shared below.<br>Hope you are able to employ this application well and get your desired output.<br> Cheers!</p>',
        unsafe_allow_html=True)
    st.subheader(
        'Vishal Malik [![Repo](https://badgen.net/badge/icon/GitHub?icon=github&label)](https://github.com/vmalik25) ')